...
namespace Django.VehiclePhysics2.Modules.Aerodynamics
{
    /// <summary>
    ///     Aerodynamics module.
    ...
    /// </summary>
    [Serializable]
    public partial class AerodynamicsModule : VehicleComponent
    {
        /// <summary>
        ///     Density of air.
        /// </summary>
        ...
        public override void VC_FixedUpdate()
        {
            ...
            if (simulateDrag)
            {
                ...
                longitudinalDragForce = 0.5f * RHO * _frontalArea * frontalCd * (_forwardSpeed * _forwardSpeed) *
                                        (_forwardSpeed > 0 ? -1f : 1f);
                lateralDragForce = 0.5f * RHO * _sideArea * sideCd * (_sideSpeed * _sideSpeed) *
                                   (_sideSpeed > 0 ? -1f : 1f);
                vehicleController.vehicleRigidbody.AddRelativeForce(new Vector3(lateralDragForce, 0, longitudinalDragForce));
            }

            // Downforce
            if (simulateDownforce)
            {
                float speedPercent = vehicleController.Speed / maxDownforceSpeed;
                float forceCoeff = 1f - (1f - Mathf.Pow(speedPercent, 2f));

                foreach (DownforcePoint dp in downforcePoints)
                {
                    vehicleController.vehicleRigidbody.AddForceAtPosition(forceCoeff * dp.maxForce * -vehicleController.transform.up,
                                                            vehicleController.transform.TransformPoint(dp.position));
                }
            }
        }
        ...
        public override void VC_DrawGizmos()
        {
            foreach (DownforcePoint dp in downforcePoints)
            {
                ...
                Vector3 pos = vehicleController.transform.TransformPoint(dp.position);
                Gizmos.DrawSphere(vehicleController.transform.TransformPoint(dp.position), 0.1f);
#if UNITY_EDITOR
