...
namespace Django.VehiclePhysics2.Powertrain
{
    [Serializable]
    public partial class TransmissionComponent : PowertrainComponent
    {
        ...
        public AutomaticTransmissionDNRShiftType automaticTransmissionDNRShiftType = AutomaticTransmissionDNRShiftType.Auto;
        ...
        public override void VC_Validate(VehicleController vc)
        {
            ...
            {
                PC_LogWarning(vc, $"Gears list is empty on {vc.name}. Open the VehicleController > PWR > Transmission tab in Unity editor " +
            ...
            if (gears.Count > 2)
            {
                if (!gears.Any (g => g > 0))
                {
                    PC_LogWarning(vc, "Gears list does not have any reverse gears. Reverse gears should be added first to the gears list and " +
                                      "should be negative. Example gears list: -3, 0, 3, 2, 1.");
                }

                if (gears.All(g => g != 0))
                {
                    PC_LogWarning(vc, "There is no neutral gear. There should be one neutral gear in the gears list, placed in between the reverse" +
                                      " (negative) and forward (positive) gears. Example gears list: -3, 0, 3, 2, 1.");
                }

                if (!gears.Any(g => g < 0))
                {
                    PC_LogWarning(vc, "Gears list does not have any forward gears. Forward gears should be added after the reverse (negative)" +
            ...
            if (transmissionType == TransmissionShiftType.CVT)
            {
                if (gears.Count != 3)
                {
                    PC_LogWarning(vc, "CVT Transmission type requires 3 gears in the gears list, one reverse, one neutral and one forward. " +
                                      "E.g. -3, 0, 2.");
                }
            }
        ...
        private float CalculateNoSlipRPM()
        {
            ...
            return UnitConverter.AngularVelocityToRPM(angVelSum / vehicleController.powertrain.wheelCount) * currentGearRatio;
        }
        ...
        private void AutomaticShift(VehicleController vc)
        {
            ...
            if (variableShiftPoint)
            {
                ...
                float inclineModifier = Mathf.Clamp01(Vector3.Dot(vc.vehicleTransform.forward, Vector3.up) * inclineEffectCoeff);
                ...
                if (vehicleSpeed > 0.4f)
                {
                    ...
                    if (currentGear < forwardGearCount && _referenceShiftRPM > TargetUpshiftRPM)
                    {
                        if (!isSequential && allowUpshiftGearSkipping)
                        {
                            ...
                            while (g < forwardGearCount)
                            {
                                ...
                                float shiftDurationPadding = Mathf.Clamp01(shiftDuration) * (_targetUpshiftRPM - _targetDownshiftRPM) * 0.25f;
                    ...
                    else if (_referenceShiftRPM < TargetDownshiftRPM)
                    {
                            ...
                            if (currentGear != 1)
                            {
                                ...
                                if (g != currentGear)
                                {
                                    ShiftInto(g);
                                                                      ...
                                                                      && automaticTransmissionDNRShiftType != AutomaticTransmissionDNRShiftType.RequireShiftInput)
                            {
                                ShiftInto(0);
                            }
                        }
                        ...
                        else
                        {
                            ...
                            else if (vehicleSpeed < dnrSpeedThreshold && throttleInput < INPUT_DEADZONE && brakeInput < INPUT_DEADZONE
                                    && automaticTransmissionDNRShiftType != AutomaticTransmissionDNRShiftType.RequireShiftInput)
                            {
                                ShiftInto(0);
                            }
...
namespace Django.VehiclePhysics2.Powertrain
{
    ...
    public partial class TransmissionComponentDrawer : PowertrainComponentDrawer
    {
        ...
        public override bool OnNUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            TransmissionComponent.TransmissionShiftType type = (TransmissionComponent.TransmissionShiftType)transmissionType.enumValueIndex;
