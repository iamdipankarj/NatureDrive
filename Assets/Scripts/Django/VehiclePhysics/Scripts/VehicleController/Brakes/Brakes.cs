...
using Django.VehiclePhysics2.Powertrain;
using Django.VehiclePhysics2.Powertrain.Wheel;
using UnityEngine;
...
namespace Django.VehiclePhysics2
{
    ...
    {
        private const float NEAR_ZERO = 0.025f;
        ...
        public override void VC_FixedUpdate()
        {
            ...
            if (actuationTime < 0.001f)
            {
                actuationTime = 0.001f;
            }
            ...
            // Brake off throttle
            if (brakeOffThrottleIntensity != 0 && _throttleInput < 0.05f)
            {
            ...
            // Brake while idle 
            bool idleBrake = brakeWhileIdle && _throttleInput < NEAR_ZERO && currentGear == 0 && vehicleController.Speed < 0.3f;
            if (idleBrake)
            {
...
namespace Django.VehiclePhysics2.GroundDetection
{
    /// <summary>
