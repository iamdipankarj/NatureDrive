  ...
  public class VehicleController : MonoBehaviour {
    ...
    public Vector3 GetSidewaysForceAppPoint(WheelData wd, Vector3 contactPoint) {
      ...
      if (wd.wheel.steer && wd.steerAngle != 0.0f && Mathf.Sign(wd.steerAngle) != Mathf.Sign(wd.tireSlip.x))
        sidewaysForcePoint += (handlingBias - 0.5f) * (m_vehicleFrame.frontPosition - m_vehicleFrame.rearPosition) * wd.transform.forward;
...
#else
      bool collided = Physics.Raycast(from, path, out RaycastHit hit, path.magnitude, layerMask);
#endif
      EnableCollidersRaycast();
    ...
    static void AdjustColliderToWheelMesh(WheelCollider wheelCollider, Transform wheelTransform) {
      ...
      wheelCollider.transform.SetPositionAndRotation(wheelTransform.position + 0.5f * wheelCollider.suspensionDistance * wheelTransform.up, wheelTransform.rotation);
