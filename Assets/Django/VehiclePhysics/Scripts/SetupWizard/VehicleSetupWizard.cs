...
using System.Linq;
using Django.Common.Cameras;
using Django.Common.CoM;
using Django.Common.Input;
using Django.Common.SceneManagement;
using Django.Common.Utility;
using Django.Common.Vehicles;
using Django.VehiclePhysics.Input;
using Django.VehiclePhysics.Modules.MotorcycleModule;
using Django.VehiclePhysics.Powertrain;
using Django.VehiclePhysics.Powertrain.Wheel;
using UnityEngine;
...
namespace Django.VehiclePhysics.SetupWizard
{
    /// <summary>
    ...
    public partial class VehicleSetupWizard : MonoBehaviour
    {
            ...
            bool addCamera = true, bool addCharacterEnterExitPoints = true)
        {
            ...
            foreach (GameObject wheelObject in wheelGOs)
            {
                ...
                {
                    Django.WheelController3D.WheelController wheelController = wcGo.AddComponent<Django.WheelController3D.WheelController>();
                ...
                wheelObject.transform.position = wheelUAPI.transform.position - wheelUAPI.transform.up * INIT_SPRING_LENGTH;
        ...
        public static void RunConfiguration(VehicleController targetVC, VehicleSetupWizardPreset preset)
        {
            ...
            {
                targetVC.stateSettings = Resources.Load("Django Vehicle Physics/Defaults/SemiTruckStateSettings") as StateSettings;
            }
            else if (preset.vehicleType == VehicleSetupWizardPreset.VehicleType.Trailer)
            {
                targetVC.stateSettings = Resources.Load("Django Vehicle Physics/Defaults/TrailerStateSettings") as StateSettings;
            }
            else
            {
                targetVC.stateSettings = Resources.Load("Django Vehicle Physics/Defaults/DefaultStateSettings") as StateSettings;
            }
            ...
            targetVC.powertrain.clutch.slipTorque = enginePeakTorque * 1.5f;
        ...
        {
            return $"Django Vehicle Physics/VehicleSetupWizard/{name}";
        }


        private static TransmissionGearingProfile LoadGearingProfile(string name)
        {
            string path = $"Django Vehicle Physics/VehicleSetupWizard/GearingProfile/{name}";
